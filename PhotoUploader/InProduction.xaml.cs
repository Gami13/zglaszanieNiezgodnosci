...
public partial class InProduction : ContentPage
{
    ...
    public async void sendPhoto(object sender, EventArgs e)
    {
        ...
        var category = categoriesUI.SelectedItem;
        var subcategory = subcategoriesUI.SelectedItem;
        var desc = descAndComment.Text;
        var elementsFinFin = JsonConvert.SerializeObject(elementsFin);
        var response = JsonConvert.SerializeObject($"{{projectNumber: {num}, elements:{elementsFinFin}, place: {place}, category:{category}, subcategory:{subcategory}, description:{desc} }} ");
    ...
    private  void addElement(object sender, EventArgs e)
    {
        ...
        newElem.Number = elementNumber.Text;
        newElem.Id = tempId;
